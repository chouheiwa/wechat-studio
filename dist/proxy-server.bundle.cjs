...
var CONFIG_PATHS = [
  import_node_path.default.join(process.cwd(), "wechat-studio.yaml"),
  import_node_path.default.join(import_node_os.default.homedir(), ".wechat-studio.yaml"),
  import_node_path.default.join(import_node_os.default.homedir(), ".config", "wechat-studio", "config.yaml")
];
...
function parseYaml(filePath) {
  try {
    return jsYaml.load(import_node_fs.default.readFileSync(filePath, "utf-8")) || {};
  } catch {
    return {};
  }
}
function loadConfig(mode = "soft") {
  const cfg = { ...DEFAULTS };
  ...
  if (configFile) {
    process.stderr.write(`\u2705 \u4F7F\u7528\u914D\u7F6E\u6587\u4EF6: ${configFile.replace(import_node_os.default.homedir(), "~")}
`);
  }
  if (mode === "strict" && !cfg.proxy) {
    if (!cfg.wechatAppID || !cfg.wechatSecret) {
      throw new Error(
        "\u7F3A\u5C11\u5FAE\u4FE1\u51ED\u8BC1\u3002\u8BF7\u8BBE\u7F6E\u73AF\u5883\u53D8\u91CF WECHAT_APP_ID / WECHAT_SECRET\uFF0C\u6216\u521B\u5EFA\u914D\u7F6E\u6587\u4EF6 (~/.config/wechat-studio/config.yaml)\uFF0C\u6216\u914D\u7F6E proxy \u4F7F\u7528 Server Mode"
      );
    }
  }
  return cfg;
...
var CACHE_DIR = import_node_path2.default.join(import_node_os2.default.homedir(), ".config", "wechat-studio");
var CACHE_FILE = import_node_path2.default.join(CACHE_DIR, "upload-cache.json");
var tokenCache = { token: "", expiresAt: 0 };
function makeRequest(url, options = {}, body = null, extra = {}) {
  return new Promise((resolve, reject) => {
    const urlObj = new URL(url);
    const lib = urlObj.protocol === "https:" ? import_node_https.default : import_node_http.default;
    const reqOptions = {
...
async function remoteUploadMaterial(proxy, filePath, timeout) {
  const boundary = import_node_crypto.default.randomBytes(16).toString("hex");
...
function buildMultipartBody(filePath, boundary) {
  ...
  const ext = import_node_path2.default.extname(filename).toLowerCase().slice(1);
...
--${boundary}--\r
`);
  const fileData = import_node_fs2.default.readFileSync(filePath);
  return Buffer.concat([header, fileData, footer]);
}
async function uploadMaterial(cfg, filePath) {
...
async function createDraft(cfg, articles, label = "\u56FE\u6587\u8349\u7A3F") {
...
// scripts/proxy-server.mjs
var SECRET_HEADER = "x-proxy-secret";
...
function parseMultipartFile(body, contentType) {
  ...
  const ext = import_node_path3.default.extname(filename) || ".bin";
  const fileStart = headerEnd + 4;
  const endBoundary = Buffer.from(`\r
--${boundary}`);
  ...
  const tmpPath = import_node_path3.default.join(import_node_os3.default.tmpdir(), `wechat-proxy-${import_node_crypto2.default.randomBytes(8).toString("hex")}${ext}`);
  import_node_fs3.default.writeFileSync(tmpPath, fileData);
  return tmpPath;
}
...
function startServer() {
  ...
  const server = import_node_http2.default.createServer(async (req, res) => {
    ...
    process.stderr.write(`[${timestamp2}] ${req.method} ${req.url}
`);
    if (req.method !== "POST") {
    ...
    try {
      ...
      process.stderr.write(`[${timestamp2}] \u5904\u7406\u5931\u8D25: ${err.message}
`);
      if (!res.headersSent) {
