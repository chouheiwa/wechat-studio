...
```bash
node skills/wechat-studio/scripts/convert.mjs -i article.md -t autumn-warm
```
...
```bash
# 使用自定义提示词
node skills/wechat-studio/scripts/convert.mjs -i article.md --custom-prompt "你的提示词"
```
...
```bash
node skills/wechat-studio/scripts/upload-image.mjs /path/to/image.jpg
```
...
```bash
node skills/wechat-studio/scripts/download-upload.mjs "https://example.com/image.png"
```
...
```bash
node skills/wechat-studio/scripts/create-draft.mjs draft.json
```
...
```bash
# 指定本地图片
node skills/wechat-studio/scripts/create-image-post.mjs \
...
```bash
# 1. 生成排版 prompt
node skills/wechat-studio/scripts/convert.mjs -i article.md -t autumn-warm > prompt.json

# 2. [AI] 执行 prompt，得到 HTML

# 3. 上传文章中的图片（逐一上传，记录 media_id 和 wechat_url）
node skills/wechat-studio/scripts/upload-image.mjs images/photo.jpg

# 4. 将 HTML 中的 <!-- IMG:0 --> 替换为实际的 wechat_url

# 5. 上传封面图
node skills/wechat-studio/scripts/upload-image.mjs cover.jpg
